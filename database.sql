...
---1 . Display all students with their class name and section
  SELECT s.FullName, c.ClassName, c.Section
FROM Students s
LEFT JOIN Classes c ON s.ClassID = c.ClassID;

--2. List students ordered by name (A—Z)

SELECT FullName
FROM Students
ORDER BY FullName ASC;

--3. List students ordered by date of birth (oldest first)
SELECT FullName, DOB
FROM Students
ORDER BY DOB ASC;


--4. Display all teachers with assigned subjects
SELECT t.FullName AS TeacherName, s.SubjectName
FROM Teachers t
...
--6. Find class-wise student count
...
--7. Display students enrolled in a specific academic year
SELECT s.FullName, e.AcademicYear
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
WHERE e.AcademicYear = '2025';

--8. List students who are not enrolled in any subject
SELECT s.FullName
FROM Students s
...
--9. Display subject-wise average marks
SELECT sub.SubjectName, AVG(r.Marks) AS AverageMarks
FROM Subjects sub
INNER JOIN Results r ON sub.SubjectID = r.SubjectID
GROUP BY sub.SubjectName;

--10. Find highest marks obtained in each subject
SELECT sub.SubjectName, MAX(r.Marks) AS HighestMarks
FROM Subjects sub
INNER JOIN Results r ON sub.SubjectID = r.SubjectID
GROUP BY sub.SubjectName;

--11. List students who scored more than 60 marks
SELECT s.FullName, r.Marks
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
INNER JOIN Results r ON e.EnrollmentID = r.EnrollmentID
WHERE r.Marks > 60;

--12. Display students who failed (marks < 40)
SELECT s.FullName, r.Marks
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
INNER JOIN Results r ON e.EnrollmentID = r.EnrollmentID
WHERE r.Marks < 40;
--13. Find top 5 students based on marks (sum across subjects)
SELECT s.FullName, SUM(r.Marks) AS TotalMarks
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
INNER JOIN Results r ON e.EnrollmentID = r.EnrollmentID
GROUP BY s.FullName
...
--14. Display students whose name starts with 'A'SELECT FullName
SELECT FullName
FROM Students
WHERE FullName LIKE 'A%';


--15. Display students whose email is NULL

SELECT FullName
FROM Teachers
WHERE Email IS NULL;

--16. Show students with marks between 50 and 80
SELECT s.FullName, r.Marks
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
INNER JOIN Results r ON e.EnrollmentID = r.EnrollmentID
WHERE r.Marks BETWEEN 50 AND 80;


--17. Display total enrollments per subject
SELECT sub.SubjectName, COUNT(e.EnrollmentID) AS TotalEnrollments
FROM Subjects sub
LEFT JOIN Enrollments e ON sub.SubjectID = e.SubjectID
GROUP BY sub.SubjectName;

--18. Find students who have results but no class assigned
SELECT s.FullName
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
INNER JOIN Results r ON e.EnrollmentID = r.EnrollmentID
...
--19. Use subquery to find students with marks above overall average
SELECT s.FullName, r.Marks
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
INNER JOIN Results r ON e.EnrollmentID = r.EnrollmentID
WHERE r.Marks > (SELECT AVG(Marks) FROM Results);

--20. Use JOIN to display student name, subject name, and marks
SELECT s.FullName AS StudentName, sub.SubjectName, r.Marks
FROM Students s
INNER JOIN Enrollments e ON s.StudentID = e.StudentID
INNER JOIN Results r ON e.EnrollmentID = r.EnrollmentID
